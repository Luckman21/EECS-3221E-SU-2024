...
void cpu(process *p)
{
    ...
    if (p->currentQueue < 2)
        p->quantumRemaining--; // Decrement the time quantum remaining by 1 if in a RR queue
}
...
int main(int argc, char *argv[])
{
    ...
    while (processes_complete < numberOfProcesses)
    {
        ...
        for (int i = 0; i < execute->size; i++)
        {
            ...
            if (temp->bursts[temp->currentBurst].step == temp->bursts[temp->currentBurst].length)
            {
                ...
                if (temp->currentBurst < temp->numberOfBursts)
                {
                    ...
                    if (temp->bursts[temp->currentBurst].length == 0)
                    {
                        ...
                        if (temp->currentQueue == 0)
                            enqueueProcess(tempReady0, temp);
                        else if (temp->currentQueue == 1)
                            enqueueProcess(tempReady1, temp);
                        else
                            enqueueProcess(tempReady2, temp);
                    }
                    else
            ...
            {
                if (temp->currentQueue < 2)
                    temp->currentQueue++; // Demote a process to the next queue level
