...
void transaction(int op, int t_type, int n, BufferItem *item, int index)
{
    ...
    char operation[20]; // A placeholder char array for the operation string
    char type;          // A placeholder char to denote a producer (P) or consumer (C) thread

    // Set the string based on the op"code" passed to the function
    ...
    // Set the character based on the "boolean" passed to the function
    if      (t_type == 0) type = 'P';
    else if (t_type == 1) type = 'C';

    // Print the log to the logfile
...
void consume(int tid, BufferItem *item)
{
    ...
    { // Wait until full semaphore is acquired
        if (terminate >= in)
        {
...
int main(int argc, char *argv[])
{
    ...
    // Create an array of IN and OUT threads, based on the parameters specified in the args
    ...
    copy = open(argv[4], O_WRONLY | O_CREAT | O_TRUNC, 0666);   //Create a new (if the file doesn't exist) or truncate the existing file with write permissions, as well as 0666 permissions
 ...
 *
 * @param param The Thread ID of the thread created
 */
void *producer(void *param)
{
    ...
    while (1 == 1)
    {
        nanos();                    // Sleep a random amount of time
        read_byte(*tid, p_data);    // read_byte from original input file
        nanos();                    // Sleep a random amount of time
        produce(*tid, p_data);      // Produce the read_byte data to the ring buffer
    }

    return NULL; // Returning implicitly closes the thread, but this condition is never hit, instead we terminate once EOF is reached in read_byte
}
...
void *consumer(void *param)
{
    ...
    {
        nanos();                    // Sleep a random amount of time
        consume(*tid, c_data);      // Consume a byte of data stored in the ring buffer
        nanos();                    // Sleep a random amount of time
        write_byte(*tid, c_data);   // write_byte to copied output file
    }

    return NULL; // Returning implicitly closes the thread, but this condition is never hit, we instead terminate while trywaiting in consume
}
